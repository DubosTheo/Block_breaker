...
Menu::Menu(SceneManager &SceneManager, sf::RenderWindow &window) : _SceneManager(SceneManager)
{
    if (!_font.openFromFile("assets/fonts/ARCADECLASSIC.TTF"))
        throw std::runtime_error("Can't load font : assets/fonts/ARCADECLASSIC.TTF");
    auto text = std::make_unique<sf::Text>(_font, "Play", 50);
    text->setFillColor(sf::Color::White);
    text->setStyle(sf::Text::Bold);
    text->setPosition({window.getSize().x / 2.0f - 50, window.getSize().y / 2.0f - 50});
